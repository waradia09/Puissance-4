...
def rgb(r,g,b):
  """
  renvoie une couleur au format hexadÃ©cimal (tkinter) Ã  partir de r,g,b normalisÃ©s
  """
  assert (0<=r<=1 and 0<=g<=1 and 0<=b<=1),"r,g,b non normalisÃ©s :"+(r,g,b)
...
def on_quit():
  """
  callback lors du click sur fermeture de fenÃªtre
  """
...
def draw_grid4():
  """
  trace le plateau de jeu (grille des disques donnÃ©e par render_data["grid"])
  """
...
def draw_align(lc):
  """
  trace des points dans les cases de la liste lc (liste de coordonnÃ©es de la grille)
  """
  if render_data["stop"]: return None
...
def configure_draw(nr,nc):
  """
  modifie les dimensions de l'affichage du plateau de jeu selon les nombres de ligne nr et colonne nc (tient compte du ratio largeur/hauteur de la fenÃªtre graphique)
  """
  dim=(nr,nc)
  render_data["dim"]=dim
...
def set_draw_disk(r,c,player):
  """
  modifie la grille avec un disque player Ã  la ligne r, colonne c
  """
  render_data["grid"][r][c]=player
  
def wait_quit():
  """
  attend explicitement la fermeture de la fenÃªtre graphique
  """
...
def init_draw(w=450,h=480):
  """
  initialise la fenÃªtre graphique (canvas de largeur w et hauteur h)
  """
...
def draw_connect4(g):
  """
  trace le plateau de jeu correspondant Ã  la grille g
